#!/bin/bash
GAMEDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )/descent"
...
if id "ark" &>/dev/null || id "odroid" &>/dev/null; then
  sudo $BINDIR/oga_controls d1x-rebirth &
else
  $BINDIR/oga_controls d1x-rebirth &
fi

./d1x-rebirth -hogdir Data 2>&1 | tee -a ./log.txt

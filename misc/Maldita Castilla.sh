...
if id "ark" &>/dev/null || id "odroid" &>/dev/null; then
  ...
  sudo $BINDIR/oga_controls MalditaCastilla &
else
  patchelf --set-interpreter /usr/lib32/ld-linux-armhf.so.3 ./MalditaCastilla
  patchelf --set-interpreter /usr/lib32/ld-linux-armhf.so.3 $BINDIR/rg351p-js2xbox
  $BINDIR/rg351p-js2xbox &
  $BINDIR/oga_controls MalditaCastilla &
fi

./MalditaCastilla 2>&1 | tee -a ./log.txt

#!/bin/bash
GAMEDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )/SuperCrateBox"
...
if id "ark" &>/dev/null || id "odroid" &>/dev/null; then
  ...
  sudo $BINDIR/oga_controls SuperCrateBox &
else
  patchelf --set-interpreter /usr/lib32/ld-linux-armhf.so.3 ./SuperCrateBox
  patchelf --set-interpreter /usr/lib32/ld-linux-armhf.so.3 $BINDIR/rg351p-js2xbox
  $BINDIR/rg351p-js2xbox &
  $BINDIR/oga_controls SuperCrateBox &
fi

./SuperCrateBox 2>&1 | tee -a ./log.txt
